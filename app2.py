...
def run_wialon_uploader():
    ...
    if submit_btn:
        if not pdf_file or not excel_file or not token or not resource_id:
            st.error("Please upload both files, and enter token and resource ID.")
        else:
            try:
                with st.spinner("Processing..."):
                    # Create datetime objects in UTC first
                    utc_tz = pytz.UTC
                    nairobi_tz = pytz.timezone("Africa/Nairobi")
                    
                    # Create naive datetime objects
                    start_naive = datetime.combine(selected_date, datetime.min.time()) + timedelta(hours=start_hour)
                    end_naive = datetime.combine(selected_date, datetime.min.time()) + timedelta(hours=end_hour)
                    
                    # Localize to Nairobi time
                    start_nairobi = nairobi_tz.localize(start_naive)
                    end_nairobi = nairobi_tz.localize(end_naive)
                    
                    # Convert to UTC
                    start_utc = start_nairobi.astimezone(utc_tz)
                    end_utc = end_nairobi.astimezone(utc_tz)
                    
                    # Get timestamps
                    tf = int(start_utc.timestamp())
                    tt = int(end_utc.timestamp())

                    # Debug information
                    st.write("Debug Time Info:")
                    st.write(f"Selected Start (Nairobi): {start_nairobi}")
                    st.write(f"Selected End (Nairobi): {end_nairobi}")
                    st.write(f"UTC Start: {start_utc}")
                    st.write(f"UTC End: {end_utc}")
