        ...
        if not pdf_file or not excel_file or not token or not resource_id:
            ...
            try:
                with st.spinner("Processing..."):
                    # Convert selected date and hours to UTC timestamp
                    # Nairobi is UTC+3, so we subtract 3 hours to get UTC
                    base_timestamp = int(datetime.combine(selected_date, datetime.min.time()).timestamp())
                    
                    # Convert hours to seconds and adjust for UTC+3
                    start_hour_seconds = (start_hour - 3) * 3600
                    end_hour_seconds = (end_hour - 3) * 3600
                    
                    # Calculate final timestamps
                    tf = base_timestamp + start_hour_seconds
                    tt = base_timestamp + end_hour_seconds

                    # Debug information
                    st.write("Debug Time Info:")
                    st.write(f"Selected Date: {selected_date}")
                    st.write(f"Start Hour (Nairobi): {start_hour}:00")
                    st.write(f"End Hour (Nairobi): {end_hour}:00")
                    st.write(f"Start Time (UTC): {datetime.fromtimestamp(tf).strftime('%Y-%m-%d %H:%M:%S')}")
                    st.write(f"End Time (UTC): {datetime.fromtimestamp(tt).strftime('%Y-%m-%d %H:%M:%S')}")
